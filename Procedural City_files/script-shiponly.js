...
// console.log("THREE BUFFER UTILS functions!!!!")
// // console.log(THREE.BufferGeometryUtils); // This should not be undefined
// var allFunctions = [];
// for (var key in THREE.BufferGeometryUtils) {
//   if (typeof THREE.BufferGeometryUtils[key] === 'function') {
//     allFunctions.push(key);
//   }
// }
// console.log(allFunctions);  // This will print a list of function names
...
// var shipMat = new THREE.MeshPhongMaterial({color:0x999999, shading:true});
var shipMat = new THREE.MeshBasicMaterial({ color: 0x999999 });
...
function replaceBuilding(x, y, index) {
    var building = allBuildings[index];
    if (building) {
        ground.remove(building.object);
        building.destroy();
    }
    var newBuilding = new Building(x, y);
...
// 	// console.log(`Speed: ${speed}, Position: ${cameraTarget.position.x}, ${cameraTarget.position.z}`);
//
// 	var maxSpeed = 3.0;
// 	var minSpeed = 0.5;
// 	var accel = 0.03;
...
//
// 		// console.log(`New Position: ${cameraTarget.position.x}, ${cameraTarget.position.z}`);
//
// }

//working, but plane can leave camera view too easily
// function updateFlight() {
//   var maxSpeed = 3.0;
//   var minSpeed = 0.5;
//   var accel = 0.03;
...
function updateFlight() {
	...
	camera.position.set(0, 0.3, 2); // Relative position to cameraTarget, adjust as needed

	camera.lookAt(cameraTarget.position);

}

//ship flipping in 3d space, not affecting direction
// function updateFlight() {
//     var maxSpeed = 3.0;
//     var minSpeed = 0.5;
//     var accel = 0.03;
...
//     if (speed < minSpeed) speed = minSpeed;
//
//     // Apply velocity to move the camera target forward based on the current facing direction
//     var forward = new THREE.Vector3();
//     cameraTarget.getWorldDirection(forward);
//     forward.multiplyScalar(-speed);
//     cameraTarget.position.add(forward);
//
//     // Rotation adjustments based on user input
//     var pitchAccel = 0.001;
//     if (isPitchUp) pitchVel += pitchAccel;
//     if (isPitchDown) pitchVel -= pitchAccel;
//     pitchVel *= 0.95;
//     // Rotate around local X-axis
//     ship.rotateOnWorldAxis(new THREE.Vector3(1, 0, 0), pitchVel);
//
//     var rollAccel = 0.002;
//     if (isRollRight) rollVel += rollAccel;
//     if (isRollLeft) rollVel -= rollAccel;
...
//     ship.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), rollVel);
//
//     var yawAccel = 0.001;
//     if (isYawRight) yawVel += yawAccel;
//     if (isYawLeft) yawVel -= yawAccel;
...
//     ship.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), yawVel);
//
//     // Update camera settings to follow the ship
//     camera.fov = 60 + speed * 12;
//     camera.updateProjectionMatrix();
//     camera.lookAt(cameraTarget.position);
// }
